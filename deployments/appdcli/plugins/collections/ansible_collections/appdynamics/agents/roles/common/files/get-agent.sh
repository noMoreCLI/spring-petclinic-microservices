...
_usage() {
  ...
  echo
  echo "  download AGENT                       Agent to download (choices: sun-java | java | sun-java7 | java7 | ibm-java | machine | machine-win | machine-solaris | machine-solaris-sparcv9 | machine-aix | dotnet | dotnet-core | dotnetcore | db | db-win | apache | apache-i386 | python | python-alpine | iib | iib-aix | php-tar | php-zts-tar)"
  echo "    -v, --version  version             Version number for the supplied agent"
  echo "    -d, --dryrun                       Returns only the download URL if specified, it is recommended to use this arg for provisioning tools such as ansible, chef, etc "
  echo
}
...
download_options() {
  if [ "$1" = "sun-java7" ] || [ "$1" = "java7" ]; then
    _app_agent="jvm%2Cjava-jdk8" #_app_agent="jvm"
    _finder="sun-jvm"
    _os_platform="linux"
  elif [ "$1" = "sun-java" ] || [ "$1" = "java" ]; then
  ...
  elif [ "$1" = "machine-solaris" ]; then
    _app_agent="machine"
    _os_platform="solaris"
    _finder="machineagent-bundle-64bit-solaris"
  elif [ "$1" = "machine-solaris-sparcv9" ]; then
    _app_agent="machine"
    _os_platform="solaris-sparc,solaris"
    _finder="sparcv9"
  elif [ "$1" = "machine-aix" ]; then
      _app_agent="machine"
      _os_platform="AIX"
      _finder="MachineAgent"
  elif [ "$1" = "dotnet_msi" ]; then
    # .net agent msi for windows is filtered by '&apm=dotnet&apm_os=windows' query string parameters
    _app_agent="dotnet"
    _finder="dotNetAgentSetup64"
    _os_platform="windows"
  elif [ "$1" = "dotnet" ]; then
    # .net agnet standalone for linux is filtered by '&apm=dotnet,dotnet-core&apm_os=linux' query string parameters
    ...
    _os_platform="windows"
  elif [ "$1" = "php-tar" ] || [ "$1" = "php-zts-tar" ]; then
    _app_agent="php"
    _finder="$1"
    _os_platform="linux"
  elif [ "$1" = "python" ]; then
    ...
    _finder="python"
    _os_platform="linux"
   elif [ "$1" = "python-alpine" ]; then
    _app_agent="python"
    _finder="alpine-linux-64bit"
    _os_platform="linux,alpine-linux"
  elif [ "$1" = "apache" ]; then
    _app_agent="webserver"
    _finder="nativeWebServer-64bit-linux"
    _os_platform="linux"
  elif [ "$1" = "apache-i386" ]; then
    _app_agent="webserver"
    _finder="nativeWebServer-32bit-linux"
    _os_platform="linux"
  elif [ "$1" = "iib" ]; then
    _app_agent="iib"
    _finder="appdynamics-iib-linux-x64"
    _os_platform="linux"
  elif [ "$1" = "iib-aix" ]; then
    _app_agent="iib"
    _finder="appdynamics-iib-aix-64"
    _os_platform="linux,aix"
  else
...
get_download_url() {
  ...
  if [ "${http_response}" -ge 400 ] && [ "${http_response}" -lt 600 ]; then
  ...
  if [ -z "$d_s3_path" ] || [ "$d_s3_path" = "" ]; then
    ...
    echo "processed payload : $processed_payload"
...
do_curl() {
  if ! curl -qLO --write-out '\n%{http_code}\n' "$1" >"${HTTP_STATUS_FILE}"; then
    exit_with_error "Failed call to Downloads portal" "${ERR_NETWORK}"
  fi
...
download() {
  while [ $# -gt 0 ]; do
    ...
    -d | --dryrun)
      ...
      dryrun="true"
      ;;
    sun-java | java | sun-java7 | java7 | ibm-java | machine | machine-win | machine-solaris | machine-solaris-sparcv9 | machine-aix | dotnet_msi | dotnet | db | db-win | iib | iib-aix | apache | apache-i386 | python | python-alpine | php-tar | php-zts-tar)
  ...
  if [ "${version:-}" == 'latest'  ]; then
    #echo $(get_latest_version ${agent})
    _version=$(get_latest_version "${agent}")
  else
    _version=${version:-};
  fi

  readonly s3_download_uri=$(get_download_url "${agent}" "${_version}")
  ...
  if [ -z "$s3_download_uri" ] || [ "$s3_download_uri" = "" ]; then
    ...
    #echo "{\"version\": \"${_version}\", \"download_url\": \"${download_url}\"}"
    echo "$download_url"
    cleanup
  else
    download_url="${DEFAULT_DOWNLOAD_SITE}/${s3_download_uri}"
