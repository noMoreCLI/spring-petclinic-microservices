...
> [!TIP]
>  **It would be smart to open 4 SSH sessions to each of these nodes. You will be interacting with all of the nodes and at times will not tell you to ssh instead informing you which node to run the commands.**
...
* **`bulk-install-ma.sh`**: Machine Agent installation
    ...
    * `--extra-vars "sim_enabled=true"`: The Machine Agent activates its full Server Visibility capabilities. This means it will not only collect basic hardware and process metrics but also report them to the AppDynamics Controller in a way that allows you to see:
      * Comprehensive data about the health and performance of your physical or virtual servers.
      * AppDynamics can then correlate this infrastructure data with application performance metrics, providing a holistic view of your system. This helps you quickly identify if application issues are caused by underlying infrastructure problems.
...
> [!TIP]
>
> Load is generated by a simple conjob which is executed regularly
>
> ```bash
